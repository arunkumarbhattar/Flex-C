...
namespace Hoard {
  ...
  public:
    ...
    inline bool isValidHoardPointer(void* ptr)
    {
        auto ParentHeapVal = (ParentHeap *)_parentHeap;
        if ((size_t)ParentHeapVal >  (size_t)ptr)
        {
            //this means the pointer is not in the heap
            //so it is not a valid hoard pointer
            return false;
        }
        return true;
    }
    inline void free (void * ptr) {
        //fetch the _parentHeap from the superblock
